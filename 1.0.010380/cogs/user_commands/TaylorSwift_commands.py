...
Albums = ['Taylor Swift', 'Fearless', 'Fearless (Platinum Edition)', 'Speak Now', 'Speak Now (Deluxe Edition)', 'Speak Now - World Tour LIVE', 'Red', 'Red (Deluxe Edition)', '1989', '1989 (Deluxe Edition)']
...
class TaylorSwift_SongsCommand:
    ...
    async def _tsSONG(self, ctx, option=None):
        ...
        elif option == "album".lower():
            rand_album = random.choice(Albums)
            await self.bot.say('{0.message.author.mention}, you should listen to **{1}**.'.format(ctx, rand_album))
        else:
            await self.bot.say('{0.message.author.mention}, the available options are: `song`, `album`.'.format(ctx))
